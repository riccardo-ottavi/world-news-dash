...
exports.buildNewsPreview = buildNewsPreview;
...
const buildArticleScore = (article) => {
    const now = Date.now();
...
function buildNewsPreview(articles) {
    ...
    const keywordSources = scoredArticles.reduce((acc, article) => {
    ...
    const categoryCounts = Object.entries(topArticles.reduce((acc, item) => {
    ...
    const summaryTopics = Array.from(new Set(topArticles.flatMap((item) => item.tags.filter((tag) => tag.startsWith("topic "))))).map((tag) => tag.replace("topic ", ""));
    ...
    const points = topArticles.map((item, index) => {
        ...
        const shortHighlight = item.highlight.length > 120
